...
class DreamMarketBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        for command in synced: print(f"Registered command: /{command.name}")
    async def close(self) -> None:
        if self.http_session is not None: await self.http_session.close()
        await super().close()
...
async def on_ready() -> None:
    ...
    if not bot.guilds: print('The bot is not connected to any server.')
    for guild in bot.guilds: print(f"Connected server: {guild.name} | Server ID: {guild.id}")

@bot.tree.error
async def on_app_command_error(interaction: discord.Interaction, error: discord.app_commands.AppCommandError) -> None:
    print('Slash-command error:'); traceback.print_exception(type(error),error,error.__traceback__)
    try:
        if interaction.response.is_done(): await interaction.followup.send(f"Command error: {error}",ephemeral=True)
        else: await interaction.response.send_message(f"Command error: {error}",ephemeral=True)
    except discord.HTTPException: print('Discord could not display the error.')
